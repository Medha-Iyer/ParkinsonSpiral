...
def evaluate():
    NN.eval()
    test_acc = []
    for j, (Xm,y) in enumerate(test_loader):
        batch_size = Xm.shape[0]
        Xm = Xm.to(device)
        y = y.to(device)
        yhat = NN.forward(Xm).reshape(batch_size)
        yhat = (yhat>0.5).float()
...
X_train = torch.load(os.path.join(filePath,"Xm_train1.pt"))
X_test = torch.load(os.path.join(filePath,"Xm_test.pt"))
